...
class Cinterp {
    public:
        ...
        void interpLin();
        //méthode pour interpolation lagrange
...
void Cinterp::interpSpline() {
    int n = dim - 1;
    std::vector<double> h(n), alpha(n), l(dim), mu(dim), z(dim);
    std::vector<double> a(dim), b(dim), c(dim), d(dim);
...
void Cinterp::plot() {
    ...
    // Envoyer les données brutes pour le cspline
    for (int i = 0; i < dim; i++) {
